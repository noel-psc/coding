...
// uint8_t UART_TxPacket[PAK_SIZE];
uint8_t UART_RxPacket[PAK_SIZE];
...
{
	UART_SendByte(0xFF); // 包头
	UART_SendArray(packet, len);
	UART_SendByte(0xFE); // 包尾
}
	...
	if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
	{
		...
		switch (RxState)
		{
			case 0:
				...
				break;
			case 1:
				UART_RxPacket[RxIndex] = RxData;
				RxIndex++;
				if (RxIndex >= PAK_SIZE) // 数据区满了，等待包尾
				{
					RxState = 2;
				}
				break;
			case 2:
				...
				{
					RxState = 0; // 重置状态机
					UART_RxFlag = 1; // 设置接收完成标志
				}
				break;
			default:
				RxState = 0; // 出现异常，重置状态机
				break;
		}
